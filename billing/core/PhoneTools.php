...
class PhoneTools {
    ...
    const DEF_COUNTRY_PREFIX = '380';


    protected const phoneNumberLengths = [
     ...
     * @param string $phone
     * @param string $country_prefix -- default '+380' for Ukraine
     * @return string -- formatted PhoneNumber
     * @throws \InvalidArgumentException
     */
    public static function cleaning(string $phone): string {
        ...
        if (strpos($cleaned, '+') === 0) {
            ...
            $code = self::getCountryCode($digits);

            // Проверка длины
            self::validateLength($digits, $code);
        ...
        if (preg_match('/^0\d{9}$/', $digits)) {
            ...
            self::validateLength($formatted, '380');
            return $formatted;
        }
        ...
        throw new \InvalidArgumentException("Не удалось определить формат номера. Укажите номер в международном формате, например +380121234567.");
    }
    ...
    public static function getCountryCode(string $number): string {
        ...
        if (!is_array(self::phoneNumberLengths)) {
        ...
        for ($i = 3; $i >= 1; $i--) {
            ...
            if (array_key_exists($countryPrefix, self::phoneNumberLengths)) {
    ...
    public static function simpleCleaning(string $phone): string {
        ...
        if (strpos($cleaned, '+') === 0) {
            ...
            if (strlen($number) < 12) {
                throw new \InvalidArgumentException("Недостаточно цифр после кода страны. Убедитесь, что номер полный: например, +380121234567.");
            }
