...
class MonoCard
{
     ...
     * Поля транзакций
     */
    const F_ID               = 'id'; 	                // string          Унікальний id транзакції
    ...
    const DESCRIPTIONS = [
        ...
        Lang::C_EN => [
            self::F_ID               => 'Unique transaction id',
        ...
        Lang::C_RU => [
            self::F_ID               => 'Уникальный id транзакции',
        ...
        Lang::C_UK => [
            self::F_ID               => 'Унікальний id транзакції',
    ...
    public static function description_field(string $field, string|null $lang_code = null): string {
        if ($lang_code === null) {
            $lang_code = Lang::code();
        }
    ...
    public static function normalize_client_info(array &$client_info): void {
        foreach ($client_info['accounts'] as &$card) {
    ...
    public static function get_statements(string $token, int $date1, int $date2, string $api_url): array {
        ...
        $request_url = $api_url.""."0"."/".$date1."/".($date2+(1*24*60*60)); // +1 день чтобы включить date2 в выборку
    ...
    public static function get_view_field(string $field, array &$statement): string {
        ...
        if ($found_pay['on_billing']) {
            
            $s = h($found_pay["pay"][$field]);

            if ($field == Pay::F_TYPE_ID)  { $s = $s . '<span class="text-secondary small"> | ' . Pay::type_title($s). '</span>'; }
            if ($field == Pay::F_AGENT_ID) { $s = $s . '<span class="text-secondary small"> | ' . __user(user_id: $s, field: User::F_NAME_FULL). '</span>'; }

            if (in_array($field, [Pay::F_DATE, Pay::F_PAY_ACNT, Pay::F_PAY_FAKT, Pay::F_DESCRIPTION])) {

                if ($field == Pay::F_DATE)                        { $s = date('d.m.Y H:i:s', $s); }
                if (in_array($field, [Pay::F_PAY_ACNT, Pay::F_PAY_FAKT]))   { $s = number_format($s, 2); }

                if ($pay_rec[$field] == $found_pay["pay"][$field]) {
                    $s = '<span class="badge text-bg-success">'.$s.'</span>';
                } else {
                    $s = '<span class="badge text-bg-warning" title="В базе: ['.$found_pay["pay"][$field].']'.CR
            ...
            switch ($field) {
                case Pay::F_PAY_FAKT:
                case Pay::F_PAY_ACNT:
                    $s = number_format($pay_rec[$field], 2);
                    break;
                case Pay::F_DATE:
                    $s = date('d.m.Y H:i:s', $pay_rec[$field]); 
                    break;
                case Pay::F_BANK_NO:
                    $s = $pay_rec[$field];
                    break;
                case Pay::F_TYPE_ID:
                    $s = $pay_rec[$field] . '<span class="text-secondary small"> | ' . Pay::type_title($pay_rec[$field]). '</span>'; 
                    break;
                case Pay::F_AGENT_ID:
                    $s = $pay_rec[$field] . '<span class="text-secondary small"> | ' . __user(user_id: $pay_rec[$field], field: User::F_NAME_FULL). '</span>'; 
                    break;
                case Pay::F_ABON_ID:
                    $s = '<input type="number" class="form-control min-w-100px" name="'.Pay::POST_REC .'[' . Pay::F_ABON_ID . ']" value="' . ($pay_rec[$field] ?? '') . '" >';
                    break;
