#!/usr/bin/env bash
set -euo pipefail


# 
# Приводит в назначенное состояние структуру папок и файлов локально и на стороне сервера
# Создаёт нужные папки и назначает права доступа папкам и файлам.
#
#    Локально тоже, поскольку локальная структура должна быть такая-же как и на сервере за исключением служебных файлов.
#    Локально их нужно иногда приводить в порядок чтобы rsync не жаловался на расхождения в папаках.
#
# ВАЖНО:
# Должен быть исправлен или переписан в соответствии с вашими нуждами сервера
#



echo "Упорядочивание папок..."
cd "$(dirname "$0")" || { echo "Не удалось перейтив папаку скрипта"; exit 1; }
pwd



# Настройки
CLOUD_USER="${USER}"
CLOUD_GROUP="www-data"
CLOUD_MOD_FOLDERS="770"
CLOUD_MOD_FILES="660"

# Временные папки
TMP="tmp"
CACHE="cache"
LOG="log"
PURIFIER="htmlpurifier"



# -----------------------------
# Создаём нужные вложенные папки
# -----------------------------
mkdir -p \
    "${TMP}/${CACHE}" \
    "${TMP}/${LOG}" \
    "${TMP}/${PURIFIER}" \
    "storage/files" \
    "public/uploads" \
    "public/uploads/archives" \
    "public/uploads/docs" \
    "public/uploads/icons" \
    "public/uploads/images" \
    "public/uploads/media"


# -----------------------------
# 1️⃣ Установить владельца и группу для всех файлов и папок
# -----------------------------
sudo chown -R "${CLOUD_USER}:${CLOUD_GROUP}" .

# -----------------------------
# 2️⃣ Права для файлов (включая скрытые)
# -----------------------------
find . -type f -exec chmod ${CLOUD_MOD_FILES} {} +

# -----------------------------
# 3️⃣ Права для папок
# -----------------------------
find . -type d -exec chmod ${CLOUD_MOD_FOLDERS} {} +

# -----------------------------
# 4️⃣ Сделать исполняемыми ключевые скрипты
# -----------------------------
EXEC_SCRIPTS=(
    "$(realpath "$0")"
    "deploy.sh"
    "watch_and_deploy.sh"
    "add_php_header.sh"
    "search_unheaded.sh"
    "gen_lang_ctrl.sh"
    "gen_lang_dict.sh"
    "scripts/updates.sh"
    "scripts/sms_sender.sh"
    "scripts/rotate_log.sh"
    "scripts/i18n.sh"
    "codex.run.sh"
    "scripts/parse_log_attack.sh"
    "scripts/parse_log_attack2.sh"
)

for f in "${EXEC_SCRIPTS[@]}"; do
    if [ -f "$f" ]; then
        sudo chmod 770 "$f" || echo "Не удалось изменить права: $f"
    else
        echo "Не найден: $f"
    fi
done
echo "Упорядочивание папаок успешно выполнено!"
echo "...Ok."
